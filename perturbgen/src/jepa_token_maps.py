...
_OLD_T_PERTURB = '/home/stuke1/perturbgen/T_perturb'
_NEW_LPS = os.environ.get(
    'LPS_DATA',
    '/mnt/sod2-project/csb4/stuke1/perturbgen_reproduction/lps',
)


def resolve_recorded_path(path: str) -> str:
    """Ckpts still store home T_perturb paths after the sod2 move."""
    if path and Path(path).exists():
        return path
    if path and _OLD_T_PERTURB in path:
        alt = path.replace(_OLD_T_PERTURB, _NEW_LPS)
        if Path(alt).exists():
            print(f'retarget recorded path:\n  {path}\n  -> {alt}')
            return alt
    return path
