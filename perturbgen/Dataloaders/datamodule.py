...
class PerturbGenDataset(Dataset):
    def __init__(
        ...
        missing = [t for t in time_steps if f'tgt_dataset_t{t}' not in tgt_datasets]
        if missing:
            raise ValueError(
                'missing tgt datasets for time steps '
                f'{missing}; have {sorted(tgt_datasets)}'
            )
...
class PerturbGenDataModule(LightningDataModule):
    def __init__(
        self,
        src_dataset: DatasetDict,
        tgt_datasets: DatasetDict,
        ...
        Description:
        ------------
        Custom datamodule for PerturbGen tokenised data.
        """
        super().__init__()
    ...
    def setup(self, stage=None):
        ...
        if stage == 'fit' or stage is None:
            ...
            if self.condition_encodings is not None:
                ...
                self.train_dataset = PerturbGenDataset(**training_args)
                if self.val_indices is not None:
                    dataset_args['split_indices'] = self.val_indices
                    self.val_dataset = PerturbGenDataset(**dataset_args)
                else:
        ...
        if stage == 'test' or stage is None:
            ...
            if self.condition_encodings is not None:
                ...
                self.test_dataset = PerturbGenDataset(**dataset_args)
            else:
                self.test_dataset = PerturbGenDataset(**dataset_args)
